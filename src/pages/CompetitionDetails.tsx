...
export default function CompetitionDetails() {
  ...
  const { data: pageData, isLoading, error } = useCompetitionPage(id);
  ...
  const isHost = pageData?.my_membership?.is_host ?? false;
  const isMember = pageData?.my_membership?.is_member ?? false;

  // Redirect if URL uses code instead of UUID
  useEffect(() => {
    if (id && pageData?.competition?.id && !isUUID(id)) {
      // URL has code, redirect to UUID version
      navigate(`/dashboard/competicoes/${pageData.competition.id}`, { replace: true });
    }
  }, [id, pageData?.competition?.id, navigate]);
  ...
  const handleRefresh = useCallback(async () => {
    await queryClient.invalidateQueries({ queryKey: ["competition-page", id] });
  }, [queryClient, id]);
  ...
  const getMembersWithoutTeam = (): AllMember[] => {
    if (!leaderboard || !leaderboard.all_members || !leaderboard.teams) {
      return leaderboard?.all_members || [];
    }
